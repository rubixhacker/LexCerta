...
export class PostgresKeyAdministration {
	...
	async revoke(publicId: string, actorSubject: string): Promise<void> {
		await this.database.transaction(async (transaction) => {
			await requireKey(transaction, publicId);
			await transaction.query(
	...
	async rotate(
		...
		return this.database.transaction(async (transaction) => {
			const prior = await requireKey(transaction, priorId);
			const now = await transaction.now();
			if (prior.rotation_overlap_until !== null) throw new Error("key already rotated");
			if (prior.expires_at <= now) throw new Error("key unavailable");
			const overlap = new Date(
				Math.min(prior.expires_at.getTime(), now.getTime() + 7 * 86_400_000),
			);
			const expiresAt = new Date(now.getTime() + 90 * 86_400_000);
			await transaction.query(
...
async function requireKey(transaction: import("./database.js").PgTransaction, publicId: string) {
	const lock = await transaction.query(
		"SELECT public_id FROM lexcerta.api_key_admission_locks WHERE public_id = $1 FOR UPDATE",
		[publicId],
	);
	if (lock.rowCount !== 1) throw new Error("key admission authority unavailable");
	const result = await transaction.query<{
		status: string;
		expires_at: Date;
		rotation_overlap_until: Date | null;
	}>(
		"SELECT status, expires_at, rotation_overlap_until FROM lexcerta.api_keys WHERE public_id = $1 FOR UPDATE",
		[publicId],
	);
	const key = result.rows[0];
	if (key === undefined || key.status !== "active") throw new Error("key unavailable");
	return key;
}
