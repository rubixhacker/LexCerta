...
must_haves:
  truths:
    ...
    - "Server startup fails gracefully if COURTLISTENER_API_KEY is missing or invalid on Vercel (loadConfig() Zod validation surfaces clear error)"
    - "Remote MCP client can connect to deployed Vercel URL and successfully execute verify_citation tool"
  artifacts:
    - path: "api/server.ts"
...
7. Verify `loadConfig()` call in `api/server.ts` will throw a clear Zod validation error if COURTLISTENER_API_KEY is unset -- confirm by inspecting that `loadConfig()` is called at module top-level (before handler executes), so Vercel Function startup fails fast with a descriptive error
</verification>
