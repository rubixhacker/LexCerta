...
describe("calendar retention migration and sweep", () => {
	...
	it("backfills calendar anniversaries and purges exactly at each deadline", async () => {
		...
		await insertLegacyKey(marchKey, "2027-03-01T00:00:00.000Z", null);
		await insertLegacyKey(leapKey, "2030-01-01T00:00:00.000Z", "2024-02-29T00:00:00.000Z");
...
async function insertLegacyKey(
	...
	await env.DB.prepare(
		"INSERT INTO api_key_records (public_id, customer_id, environment, hmac_sha256_hex, status, issued_at, expires_at, revoked_at, retention_expires_at) VALUES (?1, ?2, 'test', ?3, ?4, ?5, ?6, ?7, NULL)",
	)
		.bind(
			publicId,
			customerId,
